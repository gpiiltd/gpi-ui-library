...
const highlightDescription = (description: string, highlightText: string[] = []) => {
    if (highlightText.length === 0) return description;
      const escapedHighlightText = highlightText.map(text => text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
      const regex = new RegExp(`\\b(${escapedHighlightText.join('|')})\\b`, 'gi'); 
    const parts = description.split(regex);
    return (
      <>
        {parts.map((part, index) =>
          highlightText.some(text => text.toLowerCase() === part.toLowerCase()) ? (
            <span key={index} className="bg-orange-100 px-1 rounded cursor-pointer ">
              {part}
            </span>
          ) : (
            part
  ...
  const TableComponent: React.FC<TableProps> = ({ rows }) => {
    return (
      <div className="overflow-x-auto">
        <Typography variant={TypographyVariant.SUBTITLE} className='mb-2'>Props description</Typography>
